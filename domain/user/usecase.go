...
func NewUseCase(repo Repository, authProvider auth.Provider, jwtService jwt.Service) *UseCase {
	return &UseCase{
		repo:         repo,
...
func (uc *UseCase) Register(ctx context.Context, req RegisterRequest) (AuthResponse, error) {
	...
	// Generate JWT token
	token, err := uc.jwtService.GenerateToken(user.ID.String(), user.Email)
	if err != nil {
